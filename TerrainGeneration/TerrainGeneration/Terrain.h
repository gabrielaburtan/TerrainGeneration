...
public:
	...
	Terrain(unsigned int pseed, float pvertexInterval, unsigned int pgridSize, float pxi, float pzi, unsigned int pBiosz, unsigned int pBioborderlen)
	{
		this->seed = pseed;
...
private:
	...
	{
		bvbuild[0] = getTheBiomeDescription(PLAIN);
		bvbuild[1] = getTheBiomeDescription(MOUNTAIN);
		bvbuild[2] = getTheBiomeDescription(WATER);
		bvbuild[3] = getTheBiomeDescription(DESERT);
		bvbuild[4] = getTheBiomeDescription(CANYON);
	}
	...
	void genVectors()
	{
		...
		for (unsigned int i = 0; i < this->gridSize - 1; i++)
		{	//vertex i
		...
		std::cout << "vertices: " << this->vertices.size() << " indices: \n" << this->indices.size();
	}
	...
	HVertexStatus vertexValuation(float x, float z)
	{
		...
		double auxfxz = (double)this->gridSize / bvbuild[r.BioTp].bFrequence;
	...
	BorderHelper borderTest(float k)
	{
		...
		BorderHelper borderRes{ true,true,0 };
		if (glm::abs(nearborder - k) < this->biomeBorderLen)
		{
			...
			if (glm::abs(nearborder2 - k) < this->biomeBorderLen)
			{
				borderRes.isBorder = true;
	...
	BiomeType getBiomeXZ(double px, double pz)
	{
		...
		int type = (areaValue * BIOMESN);
		BiomeType toTest = static_cast<BiomeType>(type);
	...
	double biomeHeightValuation(BiomeType pbtval, double bNoise)
	{
		...
		else if (pbtval == MOUNTAIN)
		{
			bNoise = glm::pow(1.5, bNoise * 7.0) * 5;
		}
		else if (pbtval == WATER)
		{
			bNoise = glm::clamp(bNoise, -0.5, 1.0);
